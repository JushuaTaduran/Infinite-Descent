...
public class LevelManager : MonoBehaviour
{
    ...
    private IEnumerator WaitForGenerationComplete()
    {
        ...
        WFCTileGeneration wfcGenerator = FindObjectOfType<WFCTileGeneration>();
        if (wfcGenerator != null)
        {
            Debug.Log("Waiting for WFC generation to complete...");
            while (wfcGenerator.iterations < wfcGenerator.roomWidth * wfcGenerator.roomHeight)
            {
                yield return null;
            }
        ...
        {
            Debug.Log("Waiting for texture generation to complete...");
            yield return new WaitForSeconds(1f);
            Debug.Log("Texture generation assumed complete!");
        }
